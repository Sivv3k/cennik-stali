...
class ImportExportAudit(Base):
    """Historia operacji importu i eksportu cennika."""

    __tablename__ = "import_export_audits"

    id: Mapped[int] = mapped_column(primary_key=True)

    # Typ operacji: "export" | "import"
    operation_type: Mapped[str] = mapped_column(String(20), index=True)

    # Informacje o pliku
    file_name: Mapped[str] = mapped_column(String(255))
    file_type: Mapped[str] = mapped_column(String(10))  # xlsx, csv

    # Typ danych: base_prices, grinding, film, modifiers, all
    data_type: Mapped[str] = mapped_column(String(50))

    # Filtry użyte przy eksporcie (JSON)
    filters_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    # Statystyki
    records_count: Mapped[int] = mapped_column(Integer, default=0)
    records_added: Mapped[int] = mapped_column(Integer, default=0)  # dla importu
    records_updated: Mapped[int] = mapped_column(Integer, default=0)  # dla importu
    records_skipped: Mapped[int] = mapped_column(Integer, default=0)  # dla importu
    ...
    def __repr__(self) -> str:
        return f"<ImportExportAudit {self.operation_type} {self.data_type} ({self.records_count} records)>"
